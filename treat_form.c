...
** Last update Thu Feb  5 16:41:03 2015 Antoine Zanardi
*/
  ...
  if (vec.z <= -0.000001 || vec.z >= 0.000001)
    {
      k = -(view.z / vec.z);
      if (k >= 0.000001)
	add_klist(kist, k, plan->color, "PLAN");
    }
}
  ...
  /* else*/ if (delta > 0)
    {
      ...
      if (k <= k2 && k >= 0.0001)
	add_klist(kist, k, ball->color, "SPHERE");
      else if (k2 >= 0.0001)
	add_klist(kist, k2, ball->color, "SPHERE");
    }
}
